    ...
    public partial class Reserva_Alimentos : Form
    {
        ...
        public void Listar()
        {
            foreach (var item in ListCardapio.selectAll())
            {
                ...
                {
                    Item_Alimentos AliItem = new Item_Alimentos(item.Preco, item.Nome, item.Image, item.Qtd, item.Ingredientes, item.Id, Company, true, item.Quantos, CompanyList.selectAll()[Company].SttsEntrega);
